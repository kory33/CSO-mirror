...
{ name = Flag.genName(name)
  ...
  override def toString: String =
           s"""FLAG $name: ${if (_available.get) "available" else "unavailable"} ${if (_interrupted) "[cancelled]" else ""} [waiter: ${_waiting.get.identity}]"""
  ...
  */
  ...
  { if (_available.get) return
    ...
    if (!_waiting.compareAndSet(null, current))
       throw new IllegalStateException(s"$current cannot await already awaited: ${this} ")
